...
在频域实现对角分块矩阵多用于检测的频域算法，比频域均衡复杂的地方在于扩频和多个用户。当初我发明这个算法的时候想，既然要做频域算法，自然需要 DFT ,于是接收信号$\tilde {\mathbf{y} }$做 DFT ,长度为 NQ :发射信号$x^{(k)}$也需要做 DFT ,长度为 N 。为了维持等式的成立，系统方程成为如下形式：
$$
...
于是，引入频域符号：
$$
\widehat{\mathbf{y}} =\mathbf{W}_{NQ} \tilde{\mathbf{y}}  \\
 \widehat{\mathbf{x}}^{(k)}=\mathbf{W}_{NQ}\mathbf{x}^{(k)} \\
\widehat{\mathbf{z}}=\mathbf{W}_{NQ} \tilde{\mathbf{z}}\\
\widehat{\mathbf{B}}^{(k)}=\mathbf{W}_{NQ}\tilde {\mathbf{B}}^{(k)}\mathbf{W}_N^{-1}
$$
得到：
$$
\widehat{\mathbf{y}} =\sum_{k=1}^{K}  \widehat {\mathbf{B}}^{(k)}\widehat{\mathbf{x}}^{(k)}+\widehat{\mathbf{ z}}
$$
由于DFT的影响，$\widehat{\mathbf{x}}^{(k)}$的方差放大了N倍，$\widehat{\mathbf{ z}}$则放大了NQ倍

> 引理
>
> 假设$\mathbf{b}^{(k)}\in \mathbb{C}^{NQ\times 1} $是矩阵$\tilde{\mathbf{B}}^{(k)}$的第一列，而且
> $$
> \widehat{\mathbf{b}}^{(k)}=\mathbf{W}_{NQ}\mathbf{b}^{(k)}=\left [ \widehat{b}_1^{(k)},\widehat{b}_2^{(k)},\cdots ,\widehat{b}_{NQ}^{(k)}  \right ]^T \in \mathbb{C}^{NQ\times 1}
> $$
> $\widehat{\mathbf{B}}^{(k)}$表示为
> $$
> \widehat{\mathbf{B}}^{(k)}=\begin{bmatrix}
...
把$\widehat{\mathbf{B}}^{(k)}$和$\widehat{\mathbf{x}}^{(k)}$拼成一个大的矩阵和矢量：
$$
\widehat{\mathbf{B}}=\left [ \widehat{\mathbf{B}}^{(1)},\widehat{\mathbf{B}}^{(2)},\cdots ,\widehat{\mathbf{B}}^{(K)} \right ] \in \mathbb{C} ^{NQ\times  NK} \\
\widehat{\mathbf{x}}=\left [ \widehat{\mathbf{x}}^{(1)T},\widehat{\mathbf{x}}^{(2)T},\cdots ,\widehat{\mathbf{x}}^{(k)T} \right ] ^T \in \mathbb{C} ^{NQ\times  NK}
$$
从而频域的系统方程为
$$
\widehat{\mathbf{y}}=\widehat{\mathbf{B}}\widehat{\mathbf{x}}+\widehat{\mathbf{z}}
$$
**对角分块矩阵**：
...
排列后的频域系统方程为
$$
\bar{\mathbf{y}}=\bar{\mathbf{B}}\bar{\mathbf{x}}+\bar{\mathbf{z}}
$$
由于$\bar{\mathbf{B}}$是一个对角分块矩阵，其中的每个小块都可以单独求解而互不影响，于是一个$NQ\times NK$的大方程组被分解为N个$Q\times K$的小方程组：
$$
\bar{\mathbf{y}}_n=\bar{\mathbf{B}}_n \bar{\mathbf{x}}_n+\bar{\mathbf{z}}_n,n=1,\cdots,N
$$
