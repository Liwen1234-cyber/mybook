...
1. 特征值的实数性
   ...
   证明：令λ和$\mathbf{u}$分别是Hermitian矩阵A的特征值和与之对应的特征向量，即$A\mathbf{u}=λ\mathbf{u}$。两边同时左乘特征向量的共轭转置，得二次型标量值函数$\mathbf{u}^TA\mathbf{u}=λ\mathbf{u}^T\mathbf{u}$，对其两边取共轭转置，得到$\mathbf{u}^TA\mathbf{u}=λ^T\mathbf{u}^T u$。注意内积$\mathbf{u}^T\mathbf{u}$总是实数，则有$λ$也一定是实数。

2. 可逆矩阵的特征对关系
   令$λ,\mathbf{u}$是Hermitian矩阵$A$的特征对。若$A$可逆，则$1/λ,\mathbf{u}$是逆矩阵$A^{-1}$的特征对。
   证明：因为$A\mathbf{u}=λ\mathbf{u}$，则对两边左乘$A^{-1}$，则有$\mathbf{u}=λA^{-1}\mathbf{u}$，所以有$λ^{-1}\mathbf{u}=A^{-1}\mathbf{u}$
...
若$λ_k$是Hermitian矩阵$A$的多重特征值，并且其多重度为 $m_k$，那么 $\mathbf{rank}(A−λ_kI)=n−m_k$，因此任何一个Hermitian矩阵都满足可对角化定理的充要条件。因此，有$U^{−1}AU=Σ$。

### 重要定理

Hermitian矩阵的所有特征向量线性无关，并且相互正交。特征矩阵$\mathbf{U}=[\mathbf{u}_1,…,\mathbf{u}_n]$是酉矩阵，满足$\mathbf{U}^{-1}=U^T$

证明过程：

1. 首先证明不同特征值对应的特征向量是相互正交的
   令$λ_1≠λ_2$是Hermitian矩阵A对应的特征值，且其对应的特征向量分别是$\mathbf{u}_1,\mathbf{u}_2$，则有：
   $$
   \mathbf{u}_2^T\mathbf{A}\mathbf{u}_1=\lambda_1\mathbf{u}_2^T\mathbf{u}_1
   $$
...
2. 对前一个式子取共轭，则有：
   $$
   \mathbf{u}_1^T\mathbf{A}\mathbf{u}_2=\lambda_1\mathbf{u}_1^T\mathbf{u}_2
   $$
   因此有：$\lambda_1\mathbf{u}_1^T\mathbf{u}_2=\lambda_2\mathbf{u}_1^T\mathbf{u}_2$，由于$λ_1≠λ_2$，所以  $\mathbf{u}_1$和 $\mathbf{u}_2$ 正交。

更进一步

对于若$n×n$矩阵A，若$λ_k$是Hermitian矩阵$A$的多重特征值，并且其多重度为 $m_k$，那么 $\mathbf{rank}(A−λ_kI)=n−m_k$，并 且$A−λ_kI$是可逆的。于是，方程$(A−λ_kI)u=0$的线性无关解。这些线性无关解是正交的。由于特征矩阵U的所有特征向量即线性无关，又相互正交，故U为酉矩阵，满足$UU^T=I$，即$U^T=U^{−1}$
...
1. 正交相似形式：
   $$
   \mathbf{U}^T\mathbf{A}\mathbf{U}=\mathbf{diag}(\lambda_1,\lambda_2,…,\lambda_n)
   $$

2. 矩阵分解形式（正交相似下的范式）：
   $$
   \mathbf{A}=\mathbf{U}Σ\mathbf{U}^T
   $$

3. 求和形式：
   $$
   \mathbf{A} = \sum_{i=1}^{n}\mathbf{\lambda}_i\mathbf{u}_i\mathbf{u}_i^T
   $$
...
$$
\mathbf{X}^T\mathbf{A}\mathbf{x}=\sum_{i=1}^n\lambda_i\left|{\mathbf{x}^T\mathbf{u}_i}\right|^2
$$
...
$\mathbf{A}^{-1}$的级数展开形式：
$$
\mathbf{A}^{-1}=\sum_{i=1}^n\mathbf{\lambda}_i^{-1}\mathbf{u}_i\mathbf{u}_i^T
$$
因此若已知$\mathbf{A}$的特征值分解，可以很容易求出$\mathbf{A}^{-1}$

## 定矩阵

给定一个Hermitian矩阵（即等于其共轭转置的复矩阵） $\mathbf{M}$ ，对于任意非零复列向量 $z$，都有$z^H\mathbf{M}z$都为正，则 $\mathbf{M}$  是正定的。负定矩阵和负半定矩阵的定义类似，非正半定且非负半定的矩阵有时称为*不定*矩阵。

### 定义

对于对称实矩阵M：
$$
M\text{ positive-definite }\quad\Longleftrightarrow\quad\mathbf{x}^\top M\mathrm{~}\mathbf{x}>0\text{ for all }\mathbf{x}\in\mathbb{R}^n\setminus\{\mathbf{0}\}
$$

$$
M\text{ positive semi-definite}\quad\Longleftrightarrow\quad\mathbf{x}^\top M\mathrm{~}\mathbf{x}\geq0\text{ for all }\mathbf{x}\in\mathbb{R}^n
$$

$$
M\text{ negative-definite }\quad\Longleftrightarrow\quad\mathbf{x}^\top M\mathrm{~}\mathbf{x}<0\text{ for all }\mathbf{x}\in\mathbb{R}^n\setminus\{\mathbf{0}\}
$$

$$
M\text{ negative semi-definite }\quad\Longleftrightarrow\quad\mathbf{x}^\top M\mathrm{~}\mathbf{x}\leq0\text{ for all }\mathbf{x}\in\mathbb{R}^n
$$

同理对Hermitian矩阵M：
$$
M\text{ positive-definite}\quad\Longleftrightarrow\quad\mathbf{z}^*M\textbf{ z}>0\text{ for all }\mathbf{z}\in\mathbb{C}^n\setminus\{\mathbf{0}\}
$$
等等...
