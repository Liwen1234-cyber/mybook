...
![](./images/Spring-ApplicationContext层级结构图.png)
...
![](./images/Spring-RedisTemplate.png)
...
![](./images/SpringMVC-获取Controller处理器.png)
  ...
  * `Object handler = getHandlerInternal(request)`：**获取映射器**，底层调用 RequestMappingInfoHandlerMapping 类的方法，又调用 AbstractHandlerMethodMapping#getHandlerInternal
    ...
    * `handlerMethod = lookupHandlerMethod(lookupPath, request)`：获取当前 HandlerMapping 中的映射规则
      ...
      * `directPathMatches = this.mappingRegistry.getMappingsByDirectPath(lookupPath)`：获取当前的映射器与当前**请求的 URI 有关的所有映射规则**
        ...
        ![](./images/SpringMVC-HandlerMapping的映射规则.png)
...
![](./images/SpringMVC-Model和Map的数据解析.png)
...
* `writeWithMessageConverters(returnValue, returnType, inputMessage, outputMessage)`：使用消息转换器进行写出
  ...
  * `acceptableTypes = getAcceptableMediaTypes(request)`：**获取浏览器支持的媒体类型，请求头字段**
  ...
  ![](./images/SpringMVC-浏览器支持接收的数据类型.png)
...
* `genericConverter.write(body, targetType, selectedMediaType, outputMessage)`：调用消息转换器的 write 方法
  ...
  * `addDefaultHeaders(headers, t, contentType)`：**设置响应头中的数据类型**
    ...
    ![](./images/SpringMVC-服务器设置数据类型.png)
...
2. 拦截内容不同： Filter 对所有访问进行增强， Interceptor 仅针对 SpringMVC 的访问进行增强  
   ...
   ![](./images/SpringMVC-过滤器和拦截器的运行机制.png)
...
* 三种判定空校验器的区别 
  ![](./images/SpringMVC-三种判定空检验器的区别.png)
...
![](./images/SpringBoot-IDEA构建工程.png)
...
* **@EnableAutoConfiguration 注解：启用 SpringBoot 的自动配置机制**
  ...
  * 从 spring-boot-autoconfigure-2.5.3.jar/META-INF/spring.factories 文件中寻找 EnableAutoConfiguration 字段，获取自动装配类，**进行条件装配，按需装配**
    ...
    ![](./images/SpringBoot-自动装配配置文件.png)
...
* **profile 激活方式**
  ...
  * 虚拟机参数：在VM options 指定：`-Dspring.profiles.active=dev`
    ...
    ![](./images/SpringBoot-profile激活方式虚拟机参数.png)
...
![Cloud-组件概览](./images/Cloud-组件概览.png)
...
![Cloud-Eureka和Dubbo对比](./images/Cloud-Eureka和Dubbo对比.png)
...
* 浏览器访问 http://localhost:7001
  ...
  ![Cloud-Eureka可视化界面](./images/Cloud-Eureka可视化界面.png)
...
![Cloud-Eureka集群原理](./images/Cloud-Eureka集群原理.png)
...
![Cloud-Eureka自我保护机制](./images/Cloud-Eureka自我保护机制.png)
...
![Cloud-Ribbon架构原理](./images/Cloud-Ribbon架构原理.png)
...
![Cloud-IRule类图](./images/Cloud-IRule类图.png)
...
* 测试报错：
  ...
  ![Cloud-OpenFeign超时错误](./images/Cloud-OpenFeign超时错误.png)!](C:\Users\Seazean\Desktop\123\Cloud-OpenFeign超时错误.png)
...
![Cloud-Hystrix熔断机制](./images/Cloud-Hystrix熔断机制.png)
...
![Cloud-Hystrix工作流程](./images/Cloud-Hystrix工作流程.png)
...
* 指标说明：
  ...
  ![Cloud-Hystrix界面图示说明](./images/Cloud-Hystrix界面图示说明.png)
...
![Cloud-Gateway工作流程](./images/Cloud-Gateway工作流程.png)
...
* 测试：正常访问成功，将时间修改到 2023-01-10T16:31:44.106+08:00[Asia/Shanghai] 之后访问失败
  ...
  ![Cloud-Gateway时间断言](./images/Cloud-Gateway时间断言.png)
...
![Cloud-Config工作原理](./images/Cloud-Config工作原理.png)
...
![Cloud-Bus工作流程](./images/Cloud-Bus工作流程.png)
...
![Cloud-Stream工作流程](./images/Cloud-Stream工作流程.png)
...
* 管理后台服务：
  ...
  ![Cloud-Nacos服务列表](./images/Cloud-Nacos服务列表.png)
...
* 新增配置，然后访问 http://localhost:3377/config/info
  ...
  ![Cloud-Nacos新增配置](./images/Cloud-Nacos新增配置.png)
...
![Cloud-Nacos配置说明](./images/Cloud-Nacos配置说明.png)
...
![Cloud-Nacos命名空间](./images/Cloud-Nacos命名空间.png)
...
![](./images/Cloud-Sentinel增加流控规则.png)
...
![](./images/Cloud-Sentinel增加熔断规则.png)
...
![](./images/Cloud-Sentinel热点参数限流.png)
...
![](./images/Cloud-Sentinel增加热点规则.png)
...
![](./images/Cloud-Sentinel增加系统规则.png)
