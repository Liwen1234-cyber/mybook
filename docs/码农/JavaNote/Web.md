...
![](./images/HTML标签input属性-name-value.png)
...
* Address already in use : JVM_Bind：端口被占用，找到占用该端口的应用
  ...
  * 进程很重要：修改自己的端口号。修改的是 Tomcat 目录下`\conf\server.xml`中的配置。
    ...
    ![](./images/Tomcat-server.xml端口配置.png)
...
![](./images/Tomcat-IDEA配置Tomcat.png)
...
* 新建工程
  ![](./images/Tomcat-IEDA新建工程.png)

* 发布工程
  ![](./images/Tomcat-IDEA发布工程.png)
...
![](./images/Nginx配置文件conf.jpg)

* main 部分
  ![](./images/Nginx配置文件main部分.jpg)

* events 部分
  ![](./images/Nginx配置文件events部分.jpg)

* server 部分
  ![](./images/Nginx配置文件server部分.jpg)
