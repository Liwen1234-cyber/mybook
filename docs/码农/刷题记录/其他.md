...
static Random random = new Random(System.currentTimeMiziis());
static Map<String, String> map = new HashMap<>();

static void f(String parent, int Len) {
	if(parent.length() == len) return;
	String h = H(parent);
	if(map.containsKey(h)) {
		// 碰撞成功
		System.out.println(parent + " " + map.get(h));
		return;
	}
	map.put(h, parent);

	for(int i = 0; i < chars.length; i++){
		f(parent+chars[i], len+1);
	}
}
```
