...
def write_markdown(lang, results, archived_contents):
    ...
    '''
    content = ''
    with open('./docs/intro.md', mode='r', encoding='utf-8') as f:
        content = f.read()
    content = convert_file_contenet(content, lang, results, archived_contents)
    with open('./docs/intro.md', mode='w', encoding='utf-8') as f:
        f.write(content)
...
def get_archived_contents():
    archived_contents = []
    archived_files = os.listdir('./docs/archived')
    for file in archived_files:
        content = ''
        with open('./docs/archived/' + file, mode='r', encoding='utf-8') as f:
            content = f.read()
...
def job():
    ...
    '''
    languages = ['', 'java', 'python', 'matlab', 'markdown', 'jupyter-notebook', 'javascript', 'go', 'c', 'c++', 'html', 'css', 'unknown']
